...
export async function exportAllDataToExcel(selectedMonth?: string, selectedGroup?: string) {
  try {

    // Fetch all data in parallel
    const [allStudents, allGrades, allAttendance, allLessons, groups, criteria] = await Promise.all([
    ...
    let studentIds = new Set(allStudents.map((s: any) => s.id));

    if (selectedGroup && selectedGroup !== 'Все группы') {
      students = allStudents.filter((student: any) => student.group?.name === selectedGroup);
      studentIds = new Set(students.map((s: any) => s.id));
    }

    // Filter data by month and students if selected
    let grades = allGrades.filter((grade: any) => studentIds.has(grade.studentId));
    let attendance = allAttendance.filter((att: any) => studentIds.has(att.studentId));
    let lessons = allLessons;

    // Filter lessons by group
    if (selectedGroup && selectedGroup !== 'Все группы') {
      const selectedGroupObj = groups.find((g: any) => g.name === selectedGroup);
      if (selectedGroupObj) {
        lessons = allLessons.filter((lesson: any) => lesson.groupId === selectedGroupObj.id);
      }
    }

    if (selectedMonth && selectedMonth !== 'Все') {
      const monthNumber = MONTH_TO_NUMBER[selectedMonth];

      // Filter grades by month
      grades = grades.filter((grade: any) => grade.month === selectedMonth);

      // Filter attendance by month (date format: DD.MM)
      if (monthNumber) {
        attendance = attendance.filter((att: any) => {
          const attMonth = att.date?.split('.')[1];
          return attMonth === monthNumber;
        });

        // Filter lessons by month (date format: DD.MM)
    ...
    // Generate file name with current date, month and group if selected
    const today = new Date();
    const dateStr = today.toLocaleDateString('ru-RU').replace(/\./g, '-');
    const monthSuffix = selectedMonth && selectedMonth !== 'Все' ? `_${selectedMonth}` : '';
    const groupSuffix = selectedGroup && selectedGroup !== 'Все группы' ? `_${selectedGroup}` : '';
    const fileName = `Данные${groupSuffix}${monthSuffix}_${dateStr}.xlsx`;
